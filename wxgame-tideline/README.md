...
这是一个全新独立实现的微信小游戏工程，题材为虚构城市“湾城”的“潮汐线”早高峰通勤。目前已完成规则核心、Canvas 渲染层、微信平台适配、12 站战役、M6 自有资源接入和自动化质量基线。

## 当前目标

- 用原生小游戏 Canvas 完成一个可离线运行的 2D 人群通行小游戏。
- 借鉴“观察人流、寻找空隙、限时进入车厢”的高层玩法，但不复制任何外部项目的代码、文案、素材或品牌表达。
- 正式战役包含 12 个关卡，按四章逐站解锁，验证移动、避让、疏导、倒计时和结算闭环。
...
├─ game.json                  # 小游戏运行配置
├─ project.config.json        # 微信开发者工具项目配置（发布前复核主体 AppID）
...
- `src/runtime/game-runtime.ts` 统一管理 `home → route → game → result` 以及成就、外观、设置页面，固定步长 ticker、暂停/恢复、重试、下一站和返回路线；`start()` 幂等，避免重复启动多条循环。
- 路线卡片、暂停、重试、下一站和返回路线使用 Canvas 布局生成的触摸命中区域；摇杆和疏导输入按固定步长送入 `GameSimulation`，车门由实际开放状态决定是否可进。
- 完成一关后写入版本化本地存档：记录局数、通关数、疏导次数、最佳分和每关最高三星数，并按顺序解锁海风门、云港、星环城。
...
- `LevelEventConfig` 让事件按关卡数据触发：雨天会收窄可行走区域，提前关门先闪烁预警 3 秒再永久阻塞旧入口，行李车以软碰撞缓慢横穿站台；开始、结束和关门记录会进入局内事件日志，HUD/站台会显示当前事件。
