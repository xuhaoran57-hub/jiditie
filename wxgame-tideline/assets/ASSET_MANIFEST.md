...
- `audio/tideline-loop.mp3`：由同目录自有 WAV 经 `scripts/compress-audio.py` / LAME 3.98.4 编码；22050Hz、96kbps、单声道、18.46 秒，保留 LAME 延迟/填充元数据。版本 2.0.1，许可证与源 WAV 一致。MP3 用于发布，WAV 保留作源文件；微信真机循环衔接仍需试听。
- `project.config.json` 排除说明文档、SVG 源稿、WAV BGM 以及已经合并到图集的 slow/phone/group 独立 PNG；这些源文件保留在仓库供再生成和复核。
